...
function publishSelectedAnnouncement() {
  ...
  /* ★ กันเผยแพร่แถวที่ยังเป็นร่าง
     ระบบไม่เคยเช็ก status ตรงนี้เลย กดผิดแถวหนึ่งครั้งจะติดจุดแดงให้ทุกคน
     "และ" ประทับ broadcastedAt ไว้ด้วย พอถึงเวลาเผยแพร่ของจริง ระบบจะข้ามฉบับนั้น
     เพราะเห็นว่าแจ้งไปแล้ว ประกาศจริงจึงไม่ถึงใครเลยโดยไม่มีอะไรฟ้อง
     (แก้เองได้ด้วยการลบค่าในช่อง broadcastedAt แล้วกดใหม่) */
  var st_ = String(a.status || '').trim();
  if (st_ !== 'published' &&
      !confirm_('ประกาศนี้ยังไม่ใช่สถานะ published',
        'แถวนี้มีสถานะ "' + (st_ || '(ว่าง)') + '"\n\n' +
        'ถ้าเผยแพร่ตอนนี้ ระบบจะติดจุดแดงให้พนักงานทุกคนทันที และประทับเวลาไว้ว่าแจ้งแล้ว\n' +
        'ทำให้ตอนเผยแพร่ของจริงภายหลัง ประกาศฉบับนี้จะถูกข้ามไปเงียบ ๆ\n\n' +
        'ยืนยันจะเผยแพร่เลยหรือไม่?')) return;
...
function repairSheetHeaders() {
  ...
  if (fixed.length) {
    msg += '✅ ซ่อมแล้ว ' + fixed.length + ' ชีต\n• ' + fixed.join('\n• ') + '\n\n' +
           ...
           '   ให้กดเมนู 🔄 ล้างจุดแดงทุกคน หรือรอ 5 นาทีแล้วลองใหม่\n\n';
  }
